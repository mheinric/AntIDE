...
void test_insert_instructions(void) {
    InstructionsVect vect; 
    instruction_vect_init(&vect, 3); 
    Instruction inst; 
    instruction_init(&inst);
    for (int i = 0; i < 10; i++)
    {
        instruction_vect_push(&vect, inst);
    }
    TEST_ASSERT_EQUAL_UINT32(10, instruction_vect_size(&vect));
    TEST_ASSERT_TRUE(vect.capacity >= instruction_vect_size(&vect));
}
