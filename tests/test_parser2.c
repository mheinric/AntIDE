...
void 
test_parse2_read_empty_file() {
    Parse2Result result = parse2_program_from_file("tests/sample_programs/empty_file.asm");
...
void 
test_parse2_read_inexistant_file() {
    Parse2Result result = parse2_program_from_file("tests/sample_programs/missing-file.asm");
...
test_parse2_single_instruction_no_arg() {
    ...
    for (int i = 0; i < NB_ITEMS; i++)
    {
        ...
        TEST_ASSERT_EQUAL_UINT64_MESSAGE(1, program2_size(&result.program), inst_str[i]);
...
int run_all_parser2_tests(void) {
    ...
    RUN_TEST(test_parse2_read_empty_file);
    RUN_TEST(test_parse2_read_inexistant_file);
